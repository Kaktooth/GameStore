...
public class GameVisitsMetric implements Metric {

  private final UserInteractionRepository userInteractionRepository;

  @Override
  public UserMetric calculateMetric(UUID gameId) {
    var visitInteractionCount = userInteractionRepository.countAllGameInteractions(gameId,
        InteractionType.VISITED).doubleValue();
    var metricGenerationDate = LocalDateTime.now();
    return new UserMetric(UUID.randomUUID(), gameId, visitInteractionCount, metricGenerationDate,
        getClass().getName());
  }
}
